...
func RetrieveTotpURI(c *gin.Context) {
	...
	_account, err := accountService.RetrieveAccountWithPassword(req.Username, req.Password, config.Database())
	...
	uri, err := mfa.RetrieveTotpURI(req.Username, _account.MFASecret)
...
func ConfirmMFA(c *gin.Context) {
	...
	accountService.EnableTOTP(req.Username, config.Database())
